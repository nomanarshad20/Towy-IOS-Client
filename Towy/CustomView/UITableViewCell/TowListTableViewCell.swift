...
class TowListTableViewCell: UITableViewCell {
    ...
    var obj : TowDatum?{
        didSet{
            ...
            self.lblTitle.text = obj?.name?.stringValue ?? ""
            self.lblTime.text = "\("\(obj?.driver_reach_time_in_minutes?.intValue ?? 0)") min"
//            self.lblTitle.text = obj?.name ?? ""
//            self.lblTitle.text = obj?.name ?? ""
            self.lblPrice.text = "\(obj?.estimated_fare?.intValue ?? 0) $"
