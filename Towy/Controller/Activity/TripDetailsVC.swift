...
class TripDetailsVC: UIViewController {
    ...
    func setUI(){
        ...
        self.lblTrip.text = "Your trip with \(obj.driver_first_name?.stringValue ?? "testDriver")"
    ...
    func setUIForTowFinder(){
        guard let obj = self.obj else{return}
        let pickUpLat = Double(obj.pick_up_latitude?.stringValue ?? "0.0") ?? 0.0
        let pickUpLong = Double(obj.pick_up_longitude?.stringValue ?? "0.0") ?? 0.0
        let dropOffLat = Double(obj.drop_off_latitude?.stringValue ?? "0.0") ?? 0.0
        let dropOffLong = Double(obj.drop_off_longitude?.stringValue ?? "0.0") ?? 0.0
