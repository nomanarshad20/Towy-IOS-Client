...
class MainMapVM: BaseVM {
    ...
    func getBookingStatus(completion:@escaping (BookingStatusCheckModel?,Error?) -> ()){
        ...
        NetworkCall(data: [:], headers: UtilitiesManager.shared.getAuthHeader(), url: nil, service: APPURL.services.passengerStatus, method: .get,isJSONRequest: false).executeQuery(){
