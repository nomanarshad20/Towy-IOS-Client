...
class ChatViewController: UIViewController {
    ...
    //var booking:BookingStatusModel? = nil
    ...
    override func viewDidLoad() {
        ...
        Auth.auth().signInAnonymously { result, error in
            HIDE_CUSTOM_LOADER()
            if error != nil{
                UtilitiesManager.shared.showAlertView(title: Key.APP_NAME, message: "Error connecting server...")
            }else{
              //  self.getdata()
            }
        }
    ...
    @IBAction func sendMessage( _ sender : UIButton){
        ...
        if txtMessage.text != ""{
            if self.bookingId != nil && self.driverId != nil{
                let paramas = ["booking_id":"\(bookingId!)","receiver_id":"\(driverId!)","sender_id":"\(UtilityManager().getId())","message":txtMessage.text!,"messageTime":Date.init().timeIntervalSince1970,"type":"1"] as [String : Any]
                
                
                ref.child("\(bookingId!)").child("messages").childByAutoId().setValue(paramas) { err, refer in
                    self.txtMessage.text = ""
                    if err == nil{
                        if self.pasengerFcm != nil{
                            PushNotificationSender().sendPushNotification(to: self.pasengerFcm!, title: Constants.APP_NAME, body: self.txtMessage.text!)
                        }
                    }
                }
    ...
    */
    ...
    func getDriverFcm(completionHandler:@escaping ( _ fcm:String?)-> Void){
        ...
        if self.bookingId != nil && self.driverId != nil{
            self.ref.child("\(bookingId!)").child("fcm").child("fcm2").observeSingleEvent(of: .value) { dataSnap in
                if dataSnap.exists(){
    ...
    func getdata(){
        if self.bookingId != nil && self.driverId != nil{
            ref.child("\(bookingId!)").child("messages").observe(.childAdded) { dataSnap in
                if dataSnap.exists(){
                    print("DataSnap",dataSnap)
                    self.messages.append(Message.init(dict: dataSnap.value as? [String:Any] ?? [:]))
                    print("messagesArr",self.messages.count)
