...
struct PastBooking : Codable {
    ...
    let cancel_reason : AnyCodableValue?
    let fine_amount : AnyCodableValue?
    ...
    init(from decoder: Decoder) throws {
        ...
        cancel_reason = try values.decodeIfPresent(AnyCodableValue.self, forKey: .cancel_reason)
        fine_amount = try values.decodeIfPresent(AnyCodableValue.self, forKey: .fine_amount)
    }
